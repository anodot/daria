...
class SchemalessSource(Builder, metaclass=ABCMeta):
    ...
    def prompt_custom_delimiter(self, default_config):
        self.source.config[self.CONFIG_CSV_CUSTOM_DELIMITER] = \
            click.prompt('Custom delimiter character', type=click.STRING,
                         default=default_config.get(self.CONFIG_CSV_CUSTOM_DELIMITER)).strip()
        if len(self.source.config[self.CONFIG_CSV_CUSTOM_DELIMITER]) != 1:
            raise SourceException(f'{self.source.config[self.CONFIG_CSV_CUSTOM_DELIMITER]} is not a character')

    def prompt_csv_type(self, default_config):
        self.source.config[self.CONFIG_CSV_TYPE] = \
            click.prompt('Delimited format type', type=click.Choice(self.csv_types),
                         default=default_config.get(self.CONFIG_CSV_TYPE, self.CONFIG_CSV_TYPE_DEFAULT))
        if self.source.config[self.CONFIG_CSV_TYPE] == self.CONFIG_CSV_TYPE_CUSTOM:
    ...
    def prompt_avro_registry(self, default_config):
        self.source.config[self.CONFIG_AVRO_SCHEMA_REGISTRY_URLS] = \
            click.prompt('Registry Urls', type=click.STRING, value_proc=lambda x: x.split(','),
                         default=default_config.get(self.CONFIG_AVRO_SCHEMA_REGISTRY_URLS))
        self.source.config[self.CONFIG_AVRO_SCHEMA_LOOKUP_MODE] = \
            click.prompt('Lookup mode', type=click.Choice(self.avro_lookup_modes),
                         default=default_config.get(self.CONFIG_AVRO_SCHEMA_LOOKUP_MODE))
        if self.source.config[self.CONFIG_AVRO_SCHEMA_LOOKUP_MODE] == self.AVRO_LOOKUP_ID:
            self.source.config[self.CONFIG_AVRO_LOOKUP_ID] = \
                click.prompt('Schema ID', type=click.STRING, default=default_config.get(self.CONFIG_AVRO_LOOKUP_ID))
        elif self.source.config[self.CONFIG_AVRO_SCHEMA_LOOKUP_MODE] == self.AVRO_LOOKUP_SUBJECT:
            self.source.config[self.CONFIG_AVRO_LOOKUP_SUBJECT] = \
                click.prompt('Schema ID', type=click.STRING,
                             default=default_config.get(self.CONFIG_AVRO_LOOKUP_SUBJECT))
        else:
            self.source.config[self.CONFIG_KEY_DESERIALIZER] = 'CONFLUENT'
            self.source.config[self.CONFIG_VALUE_DESERIALIZER] = 'CONFLUENT'

    def prompt_avro(self, default_config):
        self.source.config[self.CONFIG_AVRO_SCHEMA_SOURCE] = \
            click.prompt('Schema location', type=click.Choice(self.avro_sources),
                         default=default_config.get(self.CONFIG_AVRO_SCHEMA_SOURCE))
        if self.source.config[self.CONFIG_AVRO_SCHEMA_SOURCE] == self.AVRO_SCHEMA_SOURCE_INLINE:
            self.source.config[self.CONFIG_AVRO_SCHEMA_FILE] = \
                click.prompt('Schema file path', type=click.File(),
                             default=default_config.get(self.CONFIG_AVRO_SCHEMA_FILE))
            self.source.config[self.CONFIG_AVRO_SCHEMA] = json.dumps(
                json.load(self.source.config[self.CONFIG_AVRO_SCHEMA_FILE]))
        elif self.source.config[self.CONFIG_AVRO_SCHEMA_SOURCE] == self.AVRO_SCHEMA_SOURCE_REGISTRY:
            self.prompt_avro_registry(default_config)

    def validate_grok_file(self):
        if self.source.config.get(self.CONFIG_GROK_PATTERN_FILE) and not os.path.isfile(
                self.source.config[self.CONFIG_GROK_PATTERN_FILE]):
    ...
    def prompt_log(self, default_config):
        ...
        self.source.config[self.CONFIG_GROK_PATTERN] = \
            click.prompt('Grok pattern', type=click.STRING, default=default_config.get(self.CONFIG_GROK_PATTERN))

    def prompt_data_format(self, default_config):
        self.source.config[self.CONFIG_DATA_FORMAT] = \
            click.prompt('Data format', type=click.Choice(self.data_formats),
                         default=default_config.get(self.CONFIG_DATA_FORMAT, self.DATA_FORMAT_JSON))
        if self.source.config[self.CONFIG_DATA_FORMAT] == self.DATA_FORMAT_CSV:
            self.prompt_csv(default_config)
        elif self.source.config[self.CONFIG_DATA_FORMAT] == self.DATA_FORMAT_AVRO:
    ...
    def prompt_batch_size(self, default_config):
        ...
        self.source.config[self.CONFIG_BATCH_WAIT_TIME] = \
            click.prompt('Batch Wait Time (ms)', type=click.IntRange(1),
                         default=default_config.get(self.CONFIG_BATCH_WAIT_TIME, 1000))
    ...
    def prompt_field_mapping(self, records, previous_val):
        ...
        if not new_names:
            self.source.config[self.CONFIG_CSV_MAPPING] = {}
        ...
        self.source.config[self.CONFIG_CSV_MAPPING] = data
    ...
    def print_sample_data(self):
        ...
        if not records:
            return
        if self.source.config.get(self.CONFIG_DATA_FORMAT) == self.DATA_FORMAT_CSV:
            if self.source.config.get(self.CONFIG_CSV_HEADER_LINE) == self.CONFIG_CSV_HEADER_LINE_NO_HEADER:
                self.sample_data = map_keys(records, self.source.config.get(self.CONFIG_CSV_MAPPING, {}))
            else:
    ...
    def set_config(self, config):
        ...
        if self.source.config.get('grok_definition_file'):
            with open(self.source.config['grok_definition_file'], 'r') as f:
                self.source.config[self.CONFIG_GROK_PATTERN_DEFINITION] = f.read()
