...
AGENT_API_REQUESTS_TIME = Gauge('agent_api_requests_latency_seconds', 'Agent API requests time in seconds',
                                ['endpoint'])
# Do we need it?
AGENT_API_HEALTH_CHECK = Gauge('agent_api_health_check', 'Agent API health check')

AGENT_DB_REQUESTS_TIME = Gauge('agent_db_requests_latency_seconds', 'Agent DB requests time in seconds')
# Do we need it?
...
# TODO Summary or Counter?
SOURCE_HTTP_ERRORS = Summary('source_http_errors', 'Source HTTP errors',
                             ['streamsets_url', 'pipeline_id', 'pipeline_type'])
SCHEDULED_SCRIPTS_ERRORS = Summary('scheduled_scripts_errors', 'Scheduled scripts errors', ['script_name'])
