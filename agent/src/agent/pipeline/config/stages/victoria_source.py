...
class VictoriaScript(Stage):
    def get_config(self) -> dict:
        return {'scriptConf.params': [
            ...
            {'key': 'USERNAME', 'value': self.pipeline.source.config.get(source.VictoriaMetricsSource.USERNAME, '')},
            {'key': 'PASSWORD', 'value': self.pipeline.source.config.get(source.VictoriaMetricsSource.PASSWORD, '')},
