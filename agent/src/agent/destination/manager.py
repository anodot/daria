...
def create(
    ...
    if not result.is_err():
        destination.repository.create(result.value)
...
def edit(
    ...
    if not result.is_err():
        destination.repository.update(result.value)
...
def _build(
    ...
    if access_key:
        if not validator.destination.is_valid_access_key(access_key, destination_.url):
