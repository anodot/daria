...
class TestInflux(TestPipelineBase):
        ...
        'test_output': [
            {
                'name': 'test_influx_file_short',
                'output': 'influx.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx_file_full',
                'output': 'influx_file_full.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx_adv',
                'output': 'influx_adv.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx_file_transform',
                'output': 'influx_file_transform.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx_tag',
                'output': 'influx_tag.json',
                'pipeline_type': 'influx'
            },
        ],
        'test_output_schema': [
            {
                'name': 'test_basic',
                'output': 'influx_schema.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx_schema_query',
                'output': 'influx_schema_query.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'influx2_influxql_pipeline',
                'output': 'influx2_influxql_schema.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_basic_offset',
                'output': 'influx_offset_schema.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx2',
                'output': 'influx2_schema.json',
                'pipeline_type': 'influx2'
            },
            {
                'name': 'test_influx2_file_full',
                'output': 'influx2_file_schema.json',
                'pipeline_type': 'influx2'
            },
            {
                'name': 'test_influx2_query',
                'output': 'influx2_query.json',
                'pipeline_type': 'influx2'
            },
            {
                'name': 'test_influx_schema_tag',
                'output': 'influx_schema_tag.json',
                'pipeline_type': 'influx'
            },
            {
                'name': 'test_influx2_tag',
                'output': 'influx2_tag.json',
                'pipeline_type': 'influx2'
            },
            {
                'name': 'test_influx2_query_join',
                'output': 'influx2_query_join.json',
                'pipeline_type': 'influx2'
            },
            {
                'name': 'test_influx1_basic_auth',
                'output': 'influx1_v2_basic_auth.json',
                'pipeline_type': 'influx2'
            },
        ],
    ...
    def test_watermark(self):
        ...
        assert get_output(f'{schema_id}_watermark.json') == {
            'watermark': initial_offset + interval,
            'schemaId': schema_id
        }
